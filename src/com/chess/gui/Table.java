...
public class Table {
    ...
    private static String defaultPieceImagesPath = "../img/";

    private final Color lightTileColor = Color.decode("#FFFACD");
    private final Color darkTileColor = Color.decode("#593E1A");

    public Table() {
        this.gameFrame = new JFrame("C H E S S");
    ...
    private JMenu createFileMenu() {
        ...
        //support loading prior game saves
        final JMenuItem openPGN = new JMenuItem("Load PGN File");
        openPGN.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                System.out.println("Open up the save (PGN file)");
            }
        });
            ...
            if(highlightLegalMoves) {
                for(final Move move : pieceLegalMoves(board)) {
                    if(move.getDestinationCoordinate() == this.tileId) {
                        try {
                            add(new JLabel(new ImageIcon(ImageIO.read(new File("img/green_dot.ong")))));
                        } catch(Exception e) {
                            e.printStackTrace();
                        }
                    }
            ...
            if(board.getTile(this.tileId).isTileOccupied()) {
                try {
                    final BufferedImage image = ImageIO.read(
                            //gets images from our directory: pieceIconPath + Alliance + pieceName + .gif
                            // (WB.gif for white bishop)
